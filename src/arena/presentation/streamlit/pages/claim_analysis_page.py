...
def _win_rate_color(rate: float) -> str:
    ...
    if rate >= 0.75: return "#4CAF7D"
    if rate >= 0.55: return "#6BA87D"
    if rate >= 0.40: return "#D4A843"
    return "#C9363E"
...
def _win_rate_bar(claim_df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_vline(x=0.5, line_dash="dash", line_width=1,
                  line_color="rgba(136,136,136,0.4)",
...
def _type_heatmap(df: pd.DataFrame) -> go.Figure | None:
    ...
    fig = go.Figure(go.Heatmap(
        ...
        colorscale=[[0, "#C9363E"], [0.5, "#D4A843"], [1, "#4CAF7D"]], zmin=0, zmax=1,
...
def render_claim_analysis_page():
    ...
    st.markdown(
        ...
        'letter-spacing:-0.02em;color:var(--color-text-primary,#E8E4D9);text-align:center;'
        'margin-bottom:0.15rem;">Claim Analysis</p>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<p style="color:var(--color-text-muted, #888);margin-bottom:1.5rem;text-align:center;'
    ...
    # ── Overview metrics ─────────────────────────────────────────────────────
    _conf_str = f"{avg_conf:.0%}" if avg_conf is not None else "---"
    _meta_str = "Yes" if has_metadata else "No"
    st.markdown(f"""
    <div class="ds-stat-row">
      <div class="ds-stat"><div class="ds-stat-num">{n_total}</div><div class="ds-stat-label">Total Episodes</div></div>
      <div class="ds-stat"><div class="ds-stat-num">{n_claims}</div><div class="ds-stat-label">Unique Claims</div></div>
      <div class="ds-stat"><div class="ds-stat-num">{fc_rate:.0%}</div><div class="ds-stat-label">FC Win Rate</div></div>
      <div class="ds-stat"><div class="ds-stat-num">{_conf_str}</div><div class="ds-stat-label">Avg Confidence</div></div>
      <div class="ds-stat"><div class="ds-stat-num">{_meta_str}</div><div class="ds-stat-label">Has Metadata</div></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # ── Tabs ─────────────────────────────────────────────────────────────────
    tab_names = ["Performance", "Outcomes", "Turn Sensitivity", "Consistency", "Explorer"]
    tab_perf, tab_outcomes, tab_turns, tab_consistency, tab_explorer = st.tabs(tab_names)

    # ── TAB 1: Claim Performance ─────────────────────────────────────────────
    with tab_perf:
        st.markdown("### Claim Performance")
        st.markdown(
            '<p style="font-size:0.88rem;color:var(--color-text-muted, #888);margin-top:-0.5rem;margin-bottom:0.75rem;">'
        ...
        # Metadata breakdown (shown here when metadata exists)
        if has_metadata:
            st.markdown("---")
            ...
            for tab_ui, col, label in zip(
                ...
                with tab_ui:
                    ...
                    if not tab_df.empty:
                        # Horizontal bar chart of FC Win Rate per group
                        sorted_tab = tab_df.sort_values("episodes", ascending=True)
                        labels = sorted_tab[col].tolist()
                        # Extract numeric win rate from formatted string
                        rates = sorted_tab["fc_win_rate"].apply(
                            lambda v: float(v.replace("%", "").replace("—", "0")) / 100
                        ).tolist()
                        episodes = sorted_tab["episodes"].tolist()
                        colors = [_win_rate_color(r) for r in rates]

                        fig_meta = go.Figure(go.Bar(
                            x=rates, y=labels, orientation="h",
                            marker_color=colors,
                            text=[f"{r:.0%}  ({n} ep)" for r, n in zip(rates, episodes)],
                            textposition="outside", cliponaxis=False,
                            hovertemplate="%{y}<br>FC win rate: <b>%{x:.0%}</b><extra></extra>",
                        ))
                        fig_meta.update_layout(
                            xaxis=dict(title="FC Win Rate", range=[0, 1.25],
                                       tickformat=".0%", tickfont=dict(size=11)),
                            yaxis=dict(tickfont=dict(size=11), automargin=True),
                        ...
                        fig_meta.add_vline(
                            x=0.5, line_dash="dash", line_width=1,
                            line_color="rgba(136,136,136,0.4)",
                        )
    ...
    with tab_turns:
        ...
        if not turn_df.empty and len(turn_df) > 1:
            ...
            st.info("Not enough turn data to analyze. Run debates with different turn limits to see this analysis.")
    ...
    with tab_consistency:
        if not df.empty and "claim" in df.columns:
            ...
            if _multi_claims:
                ...
                st.info(
                    "No claims have been debated more than once yet. "
                    "Run the same claim multiple times (different models or turn counts) to see consistency analysis."
                )
        else:
            st.info("No episode data available.")

    # ── TAB 5: Episode Explorer ──────────────────────────────────────────────
    with tab_explorer:
